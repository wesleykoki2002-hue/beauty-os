...
--   - Resolve 18 exact, verified, one-row ingredient identities.
--   - Create 18 missing canonical identity seeds.
--   - Create 18 exact Japanese source-label aliases.
...
--   - Governed Batch 2B read-only collision gate.
--   - Verified direct Japanese-label identity mappings.
--   - No botanical, polymer, salt-variant, fragrance, or proprietary-name inference.
--
...
create temporary table tmp_bdna_ing_0004_batch_2b_launch_products (
  product_id uuid primary key
) on commit drop;

insert into tmp_bdna_ing_0004_batch_2b_launch_products (product_id)
values
...
create temporary table tmp_bdna_ing_0004_batch_2b_identity_map (
...
insert into tmp_bdna_ing_0004_batch_2b_identity_map (
  source_name,
...
values
  ('セリン', 'セリン', 'Serine', 'serine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('グリシン', 'グリシン', 'Glycine', 'glycine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('グルタミン酸', 'グルタミン酸', 'Glutamic Acid', 'glutamic acid', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('アラニン', 'アラニン', 'Alanine', 'alanine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('アルギニン', 'アルギニン', 'Arginine', 'arginine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('トレオニン', 'トレオニン', 'Threonine', 'threonine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('プロリン', 'プロリン', 'Proline', 'proline', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('タウリン', 'タウリン', 'Taurine', 'taurine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('ロイシン', 'ロイシン', 'Leucine', 'leucine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('バリン', 'バリン', 'Valine', 'valine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('イソロイシン', 'イソロイシン', 'Isoleucine', 'isoleucine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('フェニルアラニン', 'フェニルアラニン', 'Phenylalanine', 'phenylalanine', 'amino_acid', true, 'direct_amino_acid', false, 1),
  ('スクワラン', 'スクワラン', 'Squalane', 'squalane', 'emollient_lipid', true, 'direct_lipid', false, 1),
  ('フィトスフィンゴシン', 'フィトスフィンゴシン', 'Phytosphingosine', 'phytosphingosine', 'barrier_lipid', true, 'direct_barrier_lipid', false, 1),
  ('コレステロール', 'コレステロール', 'Cholesterol', 'cholesterol', 'barrier_lipid', true, 'direct_barrier_lipid', false, 1),
  ('ベヘニルアルコール', 'ベヘニルアルコール', 'Behenyl Alcohol', 'behenyl alcohol', 'fatty_alcohol_emollient', true, 'direct_fatty_alcohol', false, 1),
  ('水添レシチン', '水添レシチン', 'Hydrogenated Lecithin', 'hydrogenated lecithin', 'phospholipid_emulsifier', true, 'direct_barrier_lipid', false, 1),
  ('エチルヘキシルグリセリン', 'エチルヘキシルグリセリン', 'Ethylhexylglycerin', 'ethylhexylglycerin', 'preservative_booster_humectant', true, 'direct_identity', false, 1);
...
begin
  ...
  join tmp_bdna_ing_0004_batch_2b_launch_products lp
    on lp.product_id = p.id
  where p.approval_status <> 'archived';

  if actual_count <> 5 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 5 non-archived launch products; found %.',
      actual_count;
  end if;
  ...
  if matched_count not in (58, 76)
     or unmatched_count not in (109, 91) then
    raise exception
      'BDNA-ING-0004 batch 2B unexpected launch baseline: matched=% unmatched=%.',
      matched_count,
      unmatched_count;
  end if;
  ...
  if open_queue_count not in (107, 89)
     or in_review_queue_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2B unexpected queue baseline: open=% in_review=%.',
      open_queue_count,
  ...
  if actual_count <> 167 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 167 launch ingredient rows; found %.',
      actual_count;
  end if;
  ...
  from tmp_bdna_ing_0004_batch_2b_identity_map m
  left join lateral (
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B source-scope mismatch: %',
      mismatch_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B canonical collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B alias collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B source-label canonical collision: %',
      collision_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B unexpected product state: %',
      mismatch_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B unexpected queue state: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 58 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 58 non-target matched rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 91 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 91 non-target unmatched rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2B expected two unchanged パラベン ambiguity holds.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2B refuses to run with compatibility-rule changes.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2B refuses to use the legacy match table.';
  end if;
end
$$;

-- ------------------------------------------------------------
-- Create the 18 missing canonical identity seeds.
-- ------------------------------------------------------------
...
select
  ...
  'medium',
  'Identity normalized in BDNA-ING-0004 Batch 2B. Full Ingredient Intelligence enrichment remains pending.',
  'approved',
  'beauty_os_admin',
  'beauty_os_admin',
  'beauty_os_admin',
  ...
  jsonb_build_object(
    ...
    'identity_evidence_sources', jsonb_build_array(
      'governed Batch 2B collision gate',
...
-- ------------------------------------------------------------
-- Create the 18 exact source-label aliases.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- Resolve the 18 product-ingredient rows without changing source names.
-- ------------------------------------------------------------
...
from resolved_map rm,
     tmp_bdna_ing_0004_batch_2b_launch_products lp
...
-- ------------------------------------------------------------
-- Resolve the matching 18 review-queue rows.
-- ------------------------------------------------------------
...
set
  ...
  notes = case
    when position(
      'Resolved by BDNA-ING-0004 Batch 2B using a verified exact identity.'
      in coalesce(q.notes, '')
    ) > 0 then q.notes
    else concat_ws(
      E'\n',
...
begin
  ...
  if actual_count <> 18 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 18 approved canonical mappings; found %.',
      actual_count;
  end if;
  ...
  where i.metadata->>'build_id' = 'BDNA-ING-0004'
    and i.metadata->>'batch_id' = 'batch_2b'
    and i.metadata->>'record_type' = 'verified_identity_seed'
    and i.review_status = 'approved';

  if actual_count <> 18 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 18 newly seeded canonicals; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 18 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 18 correctly targeted aliases; found %.',
      actual_count;
  end if;
  ...
  if exists (
    ...
    raise exception
      'BDNA-ING-0004 batch 2B direct canonical unexpectedly has an alias.';
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2B product-link postcheck failed: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 18 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 18 resolved product rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 18 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 18 resolved queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 76 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 76 matched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 91 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 91 unmatched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 89 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 89 open launch queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2B expected 2 unchanged in-review rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2B modified compatibility rules unexpectedly.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2B modified the legacy match table unexpectedly.';
  end if;

  raise notice
    'BDNA-ING-0004 batch 2B verified: 18 identities, 18 new canonicals, 18 aliases, 18 resolved rows, 76/167 matched (45.51%%).';
end
$$;
