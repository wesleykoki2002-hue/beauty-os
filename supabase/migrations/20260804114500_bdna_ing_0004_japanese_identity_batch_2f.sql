...
--   - Resolve 16 exact, verified, one-row ingredient identities.
--   - Create 16 missing canonical identity seeds.
--   - Create 16 exact Japanese source-label aliases.
...
--   - Governed Batch 2F read-only collision gate.
--   - Verified direct Japanese-label identity mappings.
--   - No ambiguous EDTA, uncertain POE copolymer, quasi-drug solution suffix,
--     fragrance, undefined polysaccharide, or proprietary-blend inference.
--
...
create temporary table tmp_bdna_ing_0004_batch_2f_launch_products (
  product_id uuid primary key
) on commit drop;

insert into tmp_bdna_ing_0004_batch_2f_launch_products (product_id)
values
...
create temporary table tmp_bdna_ing_0004_batch_2f_identity_map (
...
insert into tmp_bdna_ing_0004_batch_2f_identity_map (
  source_name,
...
values
  ('アロエベラ葉エキス', 'アロエベラ葉エキス', 'Aloe Barbadensis Leaf Extract', 'aloe barbadensis leaf extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('イソステアリルグリセリルエーテル', 'イソステアリルグリセリルエーテル', 'Isostearyl Glyceryl Ether', 'isostearyl glyceryl ether', 'emollient', true, 'direct_inci', false, 1),
  ('ウンシュウミカン果皮エキス', 'ウンシュウミカン果皮エキス', 'Citrus Unshiu Peel Extract', 'citrus unshiu peel extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('カミツレ花エキス', 'カミツレ花エキス', 'Chamomilla Recutita (Matricaria) Flower Extract', 'chamomilla recutita (matricaria) flower extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('クズ根エキス', 'クズ根エキス', 'Pueraria Lobata Root Extract', 'pueraria lobata root extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('クロレラエキス', 'クロレラエキス', 'Chlorella Vulgaris Extract', 'chlorella vulgaris extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('チャ葉エキス', 'チャ葉エキス', 'Camellia Sinensis Leaf Extract', 'camellia sinensis leaf extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('トウキンセンカ花エキス', 'トウキンセンカ花エキス', 'Calendula Officinalis Flower Extract', 'calendula officinalis flower extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('トルメンチラ根エキス', 'トルメンチラ根エキス', 'Potentilla Erecta Root Extract', 'potentilla erecta root extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('ヒアルロン酸クロスポリマーNa', 'ヒアルロン酸クロスポリマーna', 'Sodium Hyaluronate Crosspolymer', 'sodium hyaluronate crosspolymer', 'humectant', true, 'direct_hyaluronate', false, 1),
  ('ヒアルロン酸ヒドロキシプロピルトリモニウム', 'ヒアルロン酸ヒドロキシプロピルトリモニウム', 'Hydroxypropyltrimonium Hyaluronate', 'hydroxypropyltrimonium hyaluronate', 'humectant_conditioning', true, 'direct_hyaluronate', false, 1),
  ('ヘキサデシロキシPGヒドロキシエチルヘキサデカナミド', 'ヘキサデシロキシpgヒドロキシエチルヘキサデカナミド', 'Cetyl-PG Hydroxyethyl Palmitamide', 'cetyl-pg hydroxyethyl palmitamide', 'barrier_lipid', true, 'direct_pseudoceramide', false, 1),
  ('ヤシ油脂肪酸アシルグルタミン酸Na', 'ヤシ油脂肪酸アシルグルタミン酸na', 'Sodium Cocoyl Glutamate', 'sodium cocoyl glutamate', 'surfactant', true, 'direct_surfactant', false, 1),
  ('ラベンダー油', 'ラベンダー油', 'Lavandula Angustifolia Oil', 'lavandula angustifolia oil', 'essential_oil', true, 'direct_botanical_oil', false, 1),
  ('ローマカミツレ花エキス', 'ローマカミツレ花エキス', 'Anthemis Nobilis Flower Extract', 'anthemis nobilis flower extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('加水分解ヒアルロン酸Na', '加水分解ヒアルロン酸na', 'Hydrolyzed Sodium Hyaluronate', 'hydrolyzed sodium hyaluronate', 'humectant', true, 'direct_hyaluronate', false, 1);
...
begin
  ...
  join tmp_bdna_ing_0004_batch_2f_launch_products lp
    on lp.product_id = p.id
  where p.approval_status <> 'archived';

  if actual_count <> 5 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 5 non-archived launch products; found %.',
      actual_count;
  end if;
  ...
  if matched_count not in (132, 148)
     or unmatched_count not in (35, 19) then
    raise exception
      'BDNA-ING-0004 batch 2F unexpected launch baseline: matched=% unmatched=%.',
      matched_count,
      unmatched_count;
  end if;
  ...
  if open_queue_count not in (33, 17)
     or in_review_queue_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2F unexpected queue baseline: open=% in_review=%.',
      open_queue_count,
  ...
  if actual_count <> 167 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 167 launch ingredient rows; found %.',
      actual_count;
  end if;
  ...
  from tmp_bdna_ing_0004_batch_2f_identity_map m
  left join lateral (
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F source-scope mismatch: %',
      mismatch_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F canonical collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F alias collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F source-label canonical collision: %',
      collision_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F unexpected product state: %',
      mismatch_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F unexpected queue state: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 132 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 132 non-target matched rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 19 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 19 non-target unmatched rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2F expected two unchanged パラベン ambiguity holds.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2F refuses to run with compatibility-rule changes.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2F refuses to use the legacy match table.';
  end if;
end
$$;
...
select
  ...
  'medium',
  'Identity normalized in BDNA-ING-0004 Batch 2F. Full Ingredient Intelligence enrichment remains pending.',
  'approved',
  'beauty_os_admin',
  'beauty_os_admin',
  'beauty_os_admin',
  ...
  jsonb_build_object(
    ...
    'identity_evidence_sources', jsonb_build_array(
      'governed Batch 2F collision gate',
...
-- ------------------------------------------------------------
-- Create the 16 exact source-label aliases.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- Resolve the 16 product-ingredient rows without changing source names.
-- ------------------------------------------------------------
...
from resolved_map rm,
     tmp_bdna_ing_0004_batch_2f_launch_products lp
...
-- ------------------------------------------------------------
-- Resolve the matching 16 review-queue rows.
-- ------------------------------------------------------------
...
set
  ...
  notes = case
    when position(
      'Resolved by BDNA-ING-0004 Batch 2F using a verified exact identity.'
      in coalesce(q.notes, '')
    ) > 0 then q.notes
    else concat_ws(
      E'\n',
...
begin
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 16 approved canonical mappings; found %.',
      actual_count;
  end if;
  ...
  where i.metadata->>'build_id' = 'BDNA-ING-0004'
    and i.metadata->>'batch_id' = 'batch_2f'
    and i.metadata->>'record_type' = 'verified_identity_seed'
    and i.review_status = 'approved';

  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 16 newly seeded canonicals; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 16 correctly targeted aliases; found %.',
      actual_count;
  end if;
  ...
  if exists (
    ...
    raise exception
      'BDNA-ING-0004 batch 2F direct canonical unexpectedly has an alias.';
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2F product-link postcheck failed: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 16 resolved product rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 16 resolved queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 148 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 148 matched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 19 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 19 unmatched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 17 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 17 open launch queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2F expected 2 unchanged in-review rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2F modified compatibility rules unexpectedly.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2F modified the legacy match table unexpectedly.';
  end if;

  raise notice
    'BDNA-ING-0004 batch 2F verified: 16 identities, 16 new canonicals, 16 aliases, 16 resolved rows, 148/167 matched (88.62%%).';
end
$$;
