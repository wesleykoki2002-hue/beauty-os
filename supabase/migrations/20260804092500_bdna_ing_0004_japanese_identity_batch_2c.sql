...
--   - Create 20 missing canonical identity seeds.
...
--   - Governed Batch 2C read-only collision gate.
--   - Verified direct Japanese-label identity mappings.
--   - No botanical, complex-polymer, quasi-drug-solution, fragrance, or proprietary-name inference.
--
...
create temporary table tmp_bdna_ing_0004_batch_2c_launch_products (
  product_id uuid primary key
) on commit drop;

insert into tmp_bdna_ing_0004_batch_2c_launch_products (product_id)
values
...
create temporary table tmp_bdna_ing_0004_batch_2c_identity_map (
...
insert into tmp_bdna_ing_0004_batch_2c_identity_map (
  source_name,
...
values
  ('イソステアリン酸', 'イソステアリン酸', 'Isostearic Acid', 'isostearic acid', 'fatty_acid_emollient', true, 'direct_identity', false, 1),
  ('イソドデカン', 'イソドデカン', 'Isododecane', 'isododecane', 'hydrocarbon_solvent_emollient', true, 'direct_identity', false, 1),
  ('イソペンチルジオール', 'イソペンチルジオール', 'Isopentyldiol', 'isopentyldiol', 'humectant_solvent', true, 'direct_identity', false, 1),
  ('オクトクリレン', 'オクトクリレン', 'Octocrylene', 'octocrylene', 'uv_filter', true, 'direct_uv_filter', false, 1),
  ('カプリルヒドロキサム酸', 'カプリルヒドロキサム酸', 'Caprylhydroxamic Acid', 'caprylhydroxamic acid', 'preservative_chelator', true, 'direct_identity', false, 1),
  ('キシリトール', 'キシリトール', 'Xylitol', 'xylitol', 'humectant_sugar_alcohol', true, 'direct_identity', false, 1),
  ('コハク酸2Na', 'コハク酸2na', 'Disodium Succinate', 'disodium succinate', 'buffering_salt', true, 'direct_salt', false, 1),
  ('ジグリセリン', 'ジグリセリン', 'Diglycerin', 'diglycerin', 'humectant', true, 'direct_identity', false, 1),
  ('シリカ', 'シリカ', 'Silica', 'silica', 'mineral_absorbent', true, 'direct_mineral', false, 1),
  ('ソルビトール', 'ソルビトール', 'Sorbitol', 'sorbitol', 'humectant_sugar_alcohol', true, 'direct_identity', false, 1),
  ('タルク', 'タルク', 'Talc', 'talc', 'mineral_absorbent', true, 'direct_mineral', false, 1),
  ('トリエトキシカプリリルシラン', 'トリエトキシカプリリルシラン', 'Triethoxycaprylylsilane', 'triethoxycaprylylsilane', 'surface_treatment_agent', true, 'direct_identity', false, 1),
  ('トリシロキサン', 'トリシロキサン', 'Trisiloxane', 'trisiloxane', 'silicone_emollient', true, 'direct_silicone', false, 1),
  ('トリメチルシロキシケイ酸', 'トリメチルシロキシケイ酸', 'Trimethylsiloxysilicate', 'trimethylsiloxysilicate', 'film_forming_silicone_resin', true, 'direct_silicone', false, 1),
  ('ヒドロキシエチルウレア', 'ヒドロキシエチルウレア', 'Hydroxyethyl Urea', 'hydroxyethyl urea', 'humectant', true, 'direct_identity', false, 1),
  ('プロパンジオール', 'プロパンジオール', 'Propanediol', 'propanediol', 'humectant_solvent', true, 'direct_identity', false, 1),
  ('ペンチレングリコール', 'ペンチレングリコール', 'Pentylene Glycol', 'pentylene glycol', 'humectant_solvent', true, 'direct_identity', false, 1),
  ('ホモサレート', 'ホモサレート', 'Homosalate', 'homosalate', 'uv_filter', true, 'direct_uv_filter', false, 1),
  ('ミリスチン酸イソプロピル', 'ミリスチン酸イソプロピル', 'Isopropyl Myristate', 'isopropyl myristate', 'ester_emollient', true, 'direct_identity', false, 1),
  ('硫酸Mg', '硫酸mg', 'Magnesium Sulfate', 'magnesium sulfate', 'mineral_salt', true, 'direct_salt', false, 1);
...
begin
  ...
  join tmp_bdna_ing_0004_batch_2c_launch_products lp
    on lp.product_id = p.id
  where p.approval_status <> 'archived';

  if actual_count <> 5 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 5 non-archived launch products; found %.',
      actual_count;
  end if;
  ...
  if matched_count not in (76, 96)
     or unmatched_count not in (91, 71) then
    raise exception
      'BDNA-ING-0004 batch 2C unexpected launch baseline: matched=% unmatched=%.',
      matched_count,
      unmatched_count;
  end if;
  ...
  if open_queue_count not in (89, 69)
     or in_review_queue_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2C unexpected queue baseline: open=% in_review=%.',
      open_queue_count,
  ...
  if actual_count <> 167 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 167 launch ingredient rows; found %.',
      actual_count;
  end if;
  ...
  from tmp_bdna_ing_0004_batch_2c_identity_map m
  left join lateral (
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C source-scope mismatch: %',
      mismatch_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C canonical collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C alias collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C source-label canonical collision: %',
      collision_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C unexpected product state: %',
      mismatch_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C unexpected queue state: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 76 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 76 non-target matched rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 71 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 71 non-target unmatched rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2C expected two unchanged パラベン ambiguity holds.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2C refuses to run with compatibility-rule changes.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2C refuses to use the legacy match table.';
  end if;
end
$$;

-- ------------------------------------------------------------
-- Create the 20 missing canonical identity seeds.
-- ------------------------------------------------------------
...
select
  ...
  'medium',
  'Identity normalized in BDNA-ING-0004 Batch 2C. Full Ingredient Intelligence enrichment remains pending.',
  'approved',
  'beauty_os_admin',
  'beauty_os_admin',
  'beauty_os_admin',
  ...
  jsonb_build_object(
    ...
    'identity_evidence_sources', jsonb_build_array(
      'governed Batch 2C collision gate',
...
from resolved_map rm,
     tmp_bdna_ing_0004_batch_2c_launch_products lp
...
set
  ...
  notes = case
    when position(
      'Resolved by BDNA-ING-0004 Batch 2C using a verified exact identity.'
      in coalesce(q.notes, '')
    ) > 0 then q.notes
    else concat_ws(
      E'\n',
...
begin
  ...
  if actual_count <> 20 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 20 approved canonical mappings; found %.',
      actual_count;
  end if;
  ...
  where i.metadata->>'build_id' = 'BDNA-ING-0004'
    and i.metadata->>'batch_id' = 'batch_2c'
    and i.metadata->>'record_type' = 'verified_identity_seed'
    and i.review_status = 'approved';

  if actual_count <> 20 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 20 newly seeded canonicals; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 20 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 20 correctly targeted aliases; found %.',
      actual_count;
  end if;
  ...
  if exists (
    ...
    raise exception
      'BDNA-ING-0004 batch 2C direct canonical unexpectedly has an alias.';
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2C product-link postcheck failed: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 20 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 20 resolved product rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 20 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 20 resolved queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 96 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 96 matched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 71 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 71 unmatched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 69 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 69 open launch queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2C expected 2 unchanged in-review rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2C modified compatibility rules unexpectedly.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2C modified the legacy match table unexpectedly.';
  end if;

  raise notice
    'BDNA-ING-0004 batch 2C verified: 20 identities, 20 new canonicals, 20 aliases, 20 resolved rows, 96/167 matched (57.49%%).';
end
$$;
