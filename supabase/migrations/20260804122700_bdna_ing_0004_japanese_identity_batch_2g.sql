...
--   - Resolve 13 exact, verified, one-row ingredient identities.
--   - Create 12 missing canonical identity seeds.
--   - Reuse the existing approved Potassium Hydroxide canonical.
--   - Create 13 exact Japanese source-label aliases.
--   - Preserve every original Japanese/source ingredient string.
--   - Keep the two パラベン ambiguity holds unchanged.
--   - Leave four ambiguous open identities unresolved.
--
-- Identity evidence basis:
--   - Governed Batch 2G read-only collision gate.
--   - Verified direct Japanese-label identity mappings.
--   - No inference for POE・ジメチコン共重合体, α-オレフィンオリゴマー,
--     エデト酸塩, or ステアリン酸POEソルビタン.
--
...
create temporary table tmp_bdna_ing_0004_batch_2g_launch_products (
  product_id uuid primary key
) on commit drop;

insert into tmp_bdna_ing_0004_batch_2g_launch_products (product_id)
values
...
create temporary table tmp_bdna_ing_0004_batch_2g_identity_map (
...
insert into tmp_bdna_ing_0004_batch_2g_identity_map (
  source_name,
...
values
  ('スイゼンジノリ多糖体', 'スイゼンジノリ多糖体', 'Aphanothece Sacrum Polysaccharides', 'aphanothece sacrum polysaccharides', 'humectant', true, 'direct_polysaccharide', false, 1),
  ('セイヨウオトギリソウ花/葉/茎エキス', 'セイヨウオトギリソウ花/葉/茎エキス', 'Hypericum Perforatum Flower/Leaf/Stem Extract', 'hypericum perforatum flower/leaf/stem extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('フユボダイジュ花エキス', 'フユボダイジュ花エキス', 'Tilia Cordata Flower Extract', 'tilia cordata flower extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('ヤグルマギク花エキス', 'ヤグルマギク花エキス', 'Centaurea Cyanus Flower Extract', 'centaurea cyanus flower extract', 'botanical_extract', true, 'direct_botanical', false, 1),
  ('ユーカリエキス', 'ユーカリエキス', 'Eucalyptus Globulus Leaf Extract', 'eucalyptus globulus leaf extract', 'botanical_extract', true, 'direct_quasi_drug_botanical', false, 1),
  ('ラウリルヒドロキシスルホベタイン液', 'ラウリルヒドロキシスルホベタイン液', 'Lauryl Hydroxysultaine', 'lauryl hydroxysultaine', 'surfactant', true, 'direct_quasi_drug_surfactant', false, 1),
  ('ラウロイルアスパラギン酸Na液', 'ラウロイルアスパラギン酸na液', 'Sodium Lauroyl Aspartate', 'sodium lauroyl aspartate', 'surfactant', true, 'direct_quasi_drug_surfactant', false, 1),
  ('ラベンダー花水', 'ラベンダー花水', 'Lavandula Angustifolia (Lavender) Flower Water', 'lavandula angustifolia (lavender) flower water', 'botanical_water', true, 'direct_botanical_water', false, 1),
  ('乳酸球菌/ヒアルロン酸発酵液', '乳酸球菌/ヒアルロン酸発酵液', 'Lactococcus/Hyaluronic Acid Ferment Filtrate', 'lactococcus/hyaluronic acid ferment filtrate', 'ferment', true, 'direct_ferment', false, 1),
  ('加水分解ヒアルロン酸アルキル（C12-13）グリセリル', '加水分解ヒアルロン酸アルキル（c12-13）グリセリル', 'C12-13 Alkyl Glyceryl Hydrolyzed Hyaluronate', 'c12-13 alkyl glyceryl hydrolyzed hyaluronate', 'humectant', true, 'direct_hyaluronate', false, 1),
  ('水酸化カリウム液（A）', '水酸化カリウム液（a）', 'Potassium Hydroxide', 'potassium hydroxide', 'ph_adjuster', true, 'direct_quasi_drug_solution', true, 1),
  ('水酸化ナトリウム液', '水酸化ナトリウム液', 'Sodium Hydroxide', 'sodium hydroxide', 'ph_adjuster', true, 'direct_quasi_drug_solution', false, 1),
  ('香料', '香料', 'Parfum', 'parfum', 'fragrance', true, 'direct_label_term', false, 1);
...
begin
  ...
  join tmp_bdna_ing_0004_batch_2g_launch_products lp
    on lp.product_id = p.id
  where p.approval_status <> 'archived';

  if actual_count <> 5 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 5 non-archived launch products; found %.',
      actual_count;
  end if;
  ...
  if matched_count not in (148, 161)
     or unmatched_count not in (19, 6) then
    raise exception
      'BDNA-ING-0004 batch 2G unexpected launch baseline: matched=% unmatched=%.',
      matched_count,
      unmatched_count;
  end if;
  ...
  if open_queue_count not in (17, 4)
     or in_review_queue_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2G unexpected queue baseline: open=% in_review=%.',
      open_queue_count,
  ...
  if actual_count <> 167 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 167 launch ingredient rows; found %.',
      actual_count;
  end if;
  ...
  from tmp_bdna_ing_0004_batch_2g_identity_map m
  left join lateral (
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G source-scope mismatch: %',
      mismatch_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G canonical collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G alias collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G source-label canonical collision: %',
      collision_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G unexpected product state: %',
      mismatch_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G unexpected queue state: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 148 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 148 non-target matched rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 6 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 6 non-target unmatched rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2G expected two unchanged パラベン ambiguity holds.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2G refuses to run with compatibility-rule changes.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2G refuses to use the legacy match table.';
  end if;
end
$$;

-- ------------------------------------------------------------
-- Create 12 missing canonical identity seeds and reuse approved Potassium Hydroxide.
-- ------------------------------------------------------------
...
select
  ...
  'medium',
  'Identity normalized in BDNA-ING-0004 Batch 2G. Full Ingredient Intelligence enrichment remains pending.',
  'approved',
  'beauty_os_admin',
  'beauty_os_admin',
  'beauty_os_admin',
  ...
  jsonb_build_object(
    ...
    'identity_evidence_sources', jsonb_build_array(
      'governed Batch 2G collision gate',
...
from resolved_map rm,
     tmp_bdna_ing_0004_batch_2g_launch_products lp
...
set
  ...
  notes = case
    when position(
      'Resolved by BDNA-ING-0004 Batch 2G using a verified exact identity.'
      in coalesce(q.notes, '')
    ) > 0 then q.notes
    else concat_ws(
      E'\n',
...
begin
  ...
  if actual_count <> 13 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 13 approved canonical mappings; found %.',
      actual_count;
  end if;
  ...
  where i.metadata->>'build_id' = 'BDNA-ING-0004'
    and i.metadata->>'batch_id' = 'batch_2g'
    and i.metadata->>'record_type' = 'verified_identity_seed'
    and i.review_status = 'approved';

  if actual_count <> 12 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 12 newly seeded canonicals; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 13 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 13 correctly targeted aliases; found %.',
      actual_count;
  end if;
  ...
  if exists (
    ...
    raise exception
      'BDNA-ING-0004 batch 2G direct canonical unexpectedly has an alias.';
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2G product-link postcheck failed: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 13 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 13 resolved product rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 13 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 13 resolved queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 161 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 161 matched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 6 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 6 unmatched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 4 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 4 open launch queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2G expected 2 unchanged in-review rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2G modified compatibility rules unexpectedly.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2G modified the legacy match table unexpectedly.';
  end if;

  raise notice
    'BDNA-ING-0004 batch 2G verified: 13 identities, 12 new canonicals, 13 aliases, 13 resolved rows, 161/167 matched (96.41%%).';
end
$$;
