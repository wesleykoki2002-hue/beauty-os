...
--   - Create 15 missing canonical identity seeds and reuse the approved Ethylhexylglycerin canonical.
...
--   - Governed Batch 2E read-only collision gate.
--   - Verified direct Japanese-label identity mappings.
--   - No ambiguous EDTA, quasi-drug solution suffix, fragrance, botanical extract, hyaluronic derivative, or proprietary-name inference.
--
...
create temporary table tmp_bdna_ing_0004_batch_2e_launch_products (
  product_id uuid primary key
) on commit drop;

insert into tmp_bdna_ing_0004_batch_2e_launch_products (product_id)
values
...
create temporary table tmp_bdna_ing_0004_batch_2e_identity_map (
...
insert into tmp_bdna_ing_0004_batch_2e_identity_map (
  source_name,
...
values
  ('PEG-10ジメチコン', 'peg-10ジメチコン', 'PEG-10 Dimethicone', 'peg-10 dimethicone', 'silicone_emulsifier', true, 'direct_inci', false, 1),
  ('PEG-9ポリジメチルシロキシエチルジメチコン', 'peg-9ポリジメチルシロキシエチルジメチコン', 'PEG-9 Polydimethylsiloxyethyl Dimethicone', 'peg-9 polydimethylsiloxyethyl dimethicone', 'silicone_emulsifier', true, 'direct_inci', false, 1),
  ('PEG/PPG-14/7ジメチルエーテル', 'peg/ppg-14/7ジメチルエーテル', 'PEG/PPG-14/7 Dimethyl Ether', 'peg/ppg-14/7 dimethyl ether', 'humectant_solvent', true, 'direct_inci', false, 1),
  ('PEG/PPG-9/2ジメチルエーテル', 'peg/ppg-9/2ジメチルエーテル', 'PEG/PPG-9/2 Dimethyl Ether', 'peg/ppg-9/2 dimethyl ether', 'humectant_solvent', true, 'direct_inci', false, 1),
  ('PPG-10メチルグルコース', 'ppg-10メチルグルコース', 'PPG-10 Methyl Glucose Ether', 'ppg-10 methyl glucose ether', 'humectant', true, 'direct_inci', false, 1),
  ('エタノール', 'エタノール', 'Ethanol', 'ethanol', 'solvent', true, 'direct_identity', false, 1),
  ('グリセリルエチルヘキシルエーテル', 'グリセリルエチルヘキシルエーテル', 'Ethylhexylglycerin', 'ethylhexylglycerin', 'preservative_support_humectant', true, 'existing_canonical_alias', true, 1),
  ('セラミドAG', 'セラミドag', 'Ceramide AG', 'ceramide ag', 'barrier_lipid', true, 'direct_inci', false, 1),
  ('セラミドAP', 'セラミドap', 'Ceramide AP', 'ceramide ap', 'barrier_lipid', true, 'direct_inci', false, 1),
  ('セラミドEOP', 'セラミドeop', 'Ceramide EOP', 'ceramide eop', 'barrier_lipid', true, 'direct_inci', false, 1),
  ('セラミドNG', 'セラミドng', 'Ceramide NG', 'ceramide ng', 'barrier_lipid', true, 'direct_inci', false, 1),
  ('ヒスチジンHCl', 'ヒスチジンhcl', 'Histidine HCl', 'histidine hcl', 'amino_acid', true, 'direct_hydrochloride', false, 1),
  ('ビスブチルジメチコンポリグリセリル-3', 'ビスブチルジメチコンポリグリセリル-3', 'Bis-Butyldimethicone Polyglyceryl-3', 'bis-butyldimethicone polyglyceryl-3', 'silicone_emulsifier', true, 'direct_inci', false, 1),
  ('ポリリシノレイン酸ポリグリセリル-6', 'ポリリシノレイン酸ポリグリセリル-6', 'Polyglyceryl-6 Polyricinoleate', 'polyglyceryl-6 polyricinoleate', 'emulsifier', true, 'direct_inci', false, 1),
  ('ラウリルベタイン', 'ラウリルベタイン', 'Lauryl Betaine', 'lauryl betaine', 'surfactant', true, 'direct_inci', false, 1),
  ('リシンHCl', 'リシンhcl', 'Lysine HCl', 'lysine hcl', 'amino_acid', true, 'direct_hydrochloride', false, 1);
...
begin
  ...
  join tmp_bdna_ing_0004_batch_2e_launch_products lp
    on lp.product_id = p.id
  where p.approval_status <> 'archived';

  if actual_count <> 5 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 5 non-archived launch products; found %.',
      actual_count;
  end if;
  ...
  if matched_count not in (116, 132)
     or unmatched_count not in (51, 35) then
    raise exception
      'BDNA-ING-0004 batch 2E unexpected launch baseline: matched=% unmatched=%.',
      matched_count,
      unmatched_count;
  end if;
  ...
  if open_queue_count not in (49, 33)
     or in_review_queue_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2E unexpected queue baseline: open=% in_review=%.',
      open_queue_count,
  ...
  if actual_count <> 167 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 167 launch ingredient rows; found %.',
      actual_count;
  end if;
  ...
  from tmp_bdna_ing_0004_batch_2e_identity_map m
  left join lateral (
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E source-scope mismatch: %',
      mismatch_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E canonical collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E alias collision: %',
      collision_details;
  end if;
  ...
  if collision_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E source-label canonical collision: %',
      collision_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E unexpected product state: %',
      mismatch_details;
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E unexpected queue state: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 116 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 116 non-target matched rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 35 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 35 non-target unmatched rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2E expected two unchanged パラベン ambiguity holds.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2E refuses to run with compatibility-rule changes.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2E refuses to use the legacy match table.';
  end if;
end
$$;

-- ------------------------------------------------------------
-- Create 15 missing canonical identity seeds and reuse approved Ethylhexylglycerin.
-- ------------------------------------------------------------
...
select
  ...
  'medium',
  'Identity normalized in BDNA-ING-0004 Batch 2E. Full Ingredient Intelligence enrichment remains pending.',
  'approved',
  'beauty_os_admin',
  'beauty_os_admin',
  'beauty_os_admin',
  ...
  jsonb_build_object(
    ...
    'identity_evidence_sources', jsonb_build_array(
      'governed Batch 2E collision gate',
...
from resolved_map rm,
     tmp_bdna_ing_0004_batch_2e_launch_products lp
...
set
  ...
  notes = case
    when position(
      'Resolved by BDNA-ING-0004 Batch 2E using a verified exact identity.'
      in coalesce(q.notes, '')
    ) > 0 then q.notes
    else concat_ws(
      E'\n',
...
begin
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 16 approved canonical mappings; found %.',
      actual_count;
  end if;
  ...
  where i.metadata->>'build_id' = 'BDNA-ING-0004'
    and i.metadata->>'batch_id' = 'batch_2e'
    and i.metadata->>'record_type' = 'verified_identity_seed'
    and i.review_status = 'approved';

  if actual_count <> 15 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 15 newly seeded canonicals; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 16 correctly targeted aliases; found %.',
      actual_count;
  end if;
  ...
  if exists (
    ...
    raise exception
      'BDNA-ING-0004 batch 2E direct canonical unexpectedly has an alias.';
  end if;
  ...
  if mismatch_details is not null then
    raise exception
      'BDNA-ING-0004 batch 2E product-link postcheck failed: %',
      mismatch_details;
  end if;
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 16 resolved product rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 16 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 16 resolved queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 132 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 132 matched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 35 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 35 unmatched launch rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 33 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 33 open launch queue rows; found %.',
      actual_count;
  end if;
  ...
  if actual_count <> 2 then
    raise exception
      'BDNA-ING-0004 batch 2E expected 2 unchanged in-review rows; found %.',
      actual_count;
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2E modified compatibility rules unexpectedly.';
  end if;
  ...
  if (
    ...
    raise exception
      'BDNA-ING-0004 batch 2E modified the legacy match table unexpectedly.';
  end if;

  raise notice
    'BDNA-ING-0004 batch 2E verified: 16 identities, 15 new canonicals, 16 aliases, 16 resolved rows, 132/167 matched (79.04%%).';
end
$$;
