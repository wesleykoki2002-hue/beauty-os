...
export function validateOfflineFixtureCatalog(
  ...
  for (const product of products) {
    ...
    if (!clean(product.source_key)) {
      blockers.push(
        `${identity}:missing_source_key`,
      );
    }

    if (!clean(product.sku)) {
      blockers.push(
        `${identity}:missing_sku`,
      );
    }

    if (!clean(product.handle)) {
      blockers.push(
        `${identity}:missing_handle`,
      );
    }
    ...
    if (!clean(product.product_title)) {
      blockers.push(
        `${identity}:missing_product_title`,
      );
    }

    if (!clean(product.product_role)) {
      blockers.push(
        `${identity}:missing_product_role`,
      );
    }

    if (
      clean(product.shopify_status) !==
        "needs_shopify_creation"
    ) {
      blockers.push(
        `${identity}:unexpected_shopify_status`,
      );
    }
    ...
    if (
      ...
      blockers.push(
        `${identity}:fixture_contains_shopify_linkage`,
      );
    }
  }
  ...
  for (
    const [field, values] of [
      [
        "source_key",
        products.map(
          (product) => clean(product.source_key),
        ),
      ],
      [
        "sku",
        products.map(
          (product) => clean(product.sku),
        ),
      ],
      [
        "handle",
        products.map(
          (product) => clean(product.handle),
        ),
      ],
    ] as const
  ) {
    for (
      const duplicate of duplicateValues(
        values.filter(Boolean),
      )
    ) {
      blockers.push(
        `duplicate_${field}:${duplicate}`,
      );
    }
  }
...
export function buildOfflineCatalogIntent(
  ...
  const productType = clean(product.category) ||
    clean(product.product_role);
  ...
  if (
    ...
    blockers.push(
      "shopify_status_not_create_candidate",
    );
  }
  ...
  if (
    ...
    blockers.push(
      "product_already_has_shopify_linkage",
    );
  }
...
export function validateReturnedShopifyLinkage(
  ...
  if (
    !isCanonicalShopifyGid(
      returned.shopify_product_id,
      "Product",
    )
  ) {
    blockers.push(
...
export function planShopifyLinkage(
  ...
  const blockers = validateReturnedShopifyLinkage(
    returned,
  );
  ...
  if (
    ...
    blockers.push(
      "existing_shopify_product_id_conflict",
    );
  }
  ...
  if (
    ...
    blockers.push(
      "existing_shopify_variant_id_conflict",
    );
  }
  ...
  for (const owner of occupied) {
    ...
    if (
      clean(owner.shopify_product_id) ===
        returnedProductId
    ) {
      blockers.push(
        `shopify_product_id_owned_by:${ownerBeautyProductId}`,
      );
    }

    if (
      clean(owner.shopify_variant_id) ===
        returnedVariantId
    ) {
      blockers.push(
        `shopify_variant_id_owned_by:${ownerBeautyProductId}`,
      );
    }
  }
